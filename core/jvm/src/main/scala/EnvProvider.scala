...
trait EnvProvider[F[_]]:
  ...
  def get[V: Parser](name: String): F[Either[Error, V]]
...
object EnvProvider:
    ...
    given Parser[FiniteDuration] with
      override def parse(env: String): Either[Error, FiniteDuration] =
        Either.catchOnly[IllegalArgumentException](Duration(env))
          .leftMap: e =>
            Error.ParsingError(e.getMessage)
          .flatMap:
  ...
  def default[F[_]: Env: MonadThrow]: EnvProvider[F] =
    new EnvProvider[F]:
      override def get[V: Parser](name: String): F[Either[Error, V]] =
        Env[F].get(name).map:
          case Some(str) => Parser[V].parse(str)
